...
namespace Instructables.Common
{
    public sealed class GifImage : Control
    {
        ...
        static async void onSourceChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (e.NewValue == null)
                return;
            var sourceUrl = e.NewValue.ToString();
            /*if (sourceUrl == "http://www.instructables.com/static/defaultIMG/default.MEDIUM.gif")
                return;*/
            var extName = System.IO.Path.GetExtension(sourceUrl);
        ...
        public GifImage()
        {
            ...
            /* displayFrame = new Image();
             displayFrame.Stretch = Stretch.UniformToFill;
             displayFrame.Width = 400.0;
             displayFrame.Height = 300.0;
        ...
        async Task decodeGif(Uri source)
        {
            ...
            try 
            {
                ...
                loginFolder = await ApplicationData.Current.LocalFolder.CreateFolderAsync("gifFolder", CreationCollisionOption.OpenIfExists);
                var fileName = System.IO.Path.GetFileName(source.ToString());
                var file = await loginFolder.CreateFileAsync(fileName, CreationCollisionOption.ReplaceExisting);
